...
{
    public partial class PefindoResult : DataPage
    {
        ...
        private void ViewData(string key)
        {
            ...
            try
            {
                DataTable dt = conn.GetDataTable("select * from pefindo_resultmatch where appid = @1", new object[] { key }, dbtimeout);
                listvaluestring.Value = "";
                for (i = 0; i <= dt.Rows.Count - 1; i++)
                {
                    string pefindoid = dt.Rows[i]["pefindoid"].ToString();
                    string pdfviewsite = "ViewPDFPefindo.aspx?";
                    ...
                    ck.Attributes.Add("onclick", "ceknik('"+ dt.Rows[i]["pefindoid"].ToString() + "',this);");
                    if ((bool)dt.Rows[i]["selected"])
                    {
                        ck.Checked = true;
                        listvaluestring.Value += pefindoid + ":1,";
                    }
                    else
                    {
                        ck.Checked = false;
                        listvaluestring.Value += pefindoid + ":0,";
                    }
                    ...
                    string urlnavigate = pdfviewsite + "pefindoid=" + pefindoid;
